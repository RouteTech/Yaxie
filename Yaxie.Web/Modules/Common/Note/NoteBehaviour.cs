...
using System.Linq;
using Yaxie.Administration.Entities;
using Yaxie.Common.Entities;
using Yaxie.Common.Repositories;

namespace Yaxie.Web.Modules.Common.Note
{
    public class NoteBehavior : BaseSaveDeleteBehavior, IImplicitBehavior, IRetrieveBehavior, IFieldBehavior
    {
        ...
        public bool ActivateFor(Row row)
        {
            ...
            var attr = Target.GetAttribute<NoteEditorAttribute>();
            if (attr == null)
                return false;
        ...
        private void DeleteNote(IUnitOfWork uow, Int64 Pknoteid)
        {
            new NoteRepository().Delete(uow, new DeleteRequest { EntityId = Pknoteid });
        }
        ...
        {
            if (!(Target.AsObject(handler.Row) is List<NoteRow> newList))
                return;
